...
# 加载预训练的YOLOv11n模型
model = YOLO(r"Z:\伊化煤矿\models\RollerThicknessDetection.pt")
# model.model.names = {0: 'dl'}   # 或者 m.names = {0:'wet'} 取决于版本
# model.save('best_wet.pt')
# 对指定的图像文件夹进行推理，并设置各种参数
results = model.predict(
    source=r"Z:/伊化煤矿/罐耳胶轮厚度检测/frame_000000.jpg",  # 数据来源，可以是文件夹、图片路径、视频、URL，或设备ID（如摄像头）
    conf=0.5,  # 置信度阈值
    iou=0.5,  # IoU 阈值
    imgsz=640,  # 图像大小
    half=False,  # 使用半精度推理
